...
class DiagonalLinear(nn.Module):
    ...
    def __init__(self, n_copies, ip_dim, op_dim, bias=True):
        ...
        self.weights1 = nn.ParameterList(
            [nn.Parameter(torch.randn(op_dim, ip_dim)) for _ in range(n_copies)]
        )

        if self.bias:
            self.b1_shape = (n_copies * op_dim,)
            self.bias1 = nn.ParameterList(
                [nn.Parameter(torch.randn(op_dim)) for _ in range(n_copies)]
            )
    ...
    def _build_weights(self):
        ...
        for i in range(self.n_copies):
            self.W1[
                i * self.op_dim : (i + 1) * self.op_dim,
                i * self.ip_dim : (i + 1) * self.ip_dim,
            ] = self.weights1[i]
