...
class MappingFFGaussian(BaseModel):
    ...
    def __init__(
        ...
        self.encoders = nn.Sequential(
            OrderedDict(
                [
                    ...
                    (
                        "diaglin2",
                        DiagonalLinear(num_features, encoder_layers, dim_z_per),
                    ),
                ]
            )
        )
            ...
            OrderedDict(
                [
                    (
                        ...
                        nn.Linear(
                            (
                                num_features * (dim_z_per + 1)
                                if augment
                                else num_features * dim_z_per
                            ),
                            decoder_layers,
                        ),
                    ),
        ...
        self.lv = nn.Sequential(
            OrderedDict([("ffnn1", nn.Linear(decoder_layers, ydim))])
        )

    def _encode(self, x, apply_block_drop=True):
        z = self.encoders(x.permute(0, 2, 1).contiguous().view(x.shape[0], -1))
        return self.block_drop(
            z.view(z.shape[0], self.num_features, self.dim_z_per).permute(0, 2, 1),
            enabled=apply_block_drop,
        )
