...
class ModelManager:
    ...
    def __init__(self, config_path, make_plots=True, is_resume=False):
        ...
        self.optimizer = self.get_instance(
            torch.optim, "optimizer", self.config, self.trainable_params
        )
        self.lr_scheduler = self.get_instance(
            torch.optim.lr_scheduler, "lr_scheduler", self.config, self.optimizer
        )
    ...
    def clean_checkpoint_files(self):
        for f in glob.glob(
            os.path.join(self.trainer.checkpoint_dir, "checkpoint-*.pth")
        ):
            os.remove(f)
    ...
    def load_model(self, checkpoint_file="model_best.pth", fullpath=False):
        if fullpath:
            ...
            checkpoint = torch.load(
                os.path.join(self.trainer.checkpoint_dir, checkpoint_file)
            )
    ...
    def process_numpy(
        self, input_data, checkpoint_file="model_best.pth", fullpath=False
    ):
