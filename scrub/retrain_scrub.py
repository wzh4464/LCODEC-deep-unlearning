...
def scrubMany(args):
    ...
    bad_sample_indices = (
        []
    )  # if restaarting (start_point ~= 0), then check runs for bad samples
    while i < args.n_removals:
        ...
        if len(bad_sample_indices) != 0:
            residual_plus_unremoved_bad_indices = residual_indices.extend(
                bad_sample_indices
            )
        else:
        ...
        residual_dataset, _ = getDatasets(
            name=args.dataset,
            val_also=False,
            include_indices=residual_plus_unremoved_bad_indices,
        )
        ...
        (
            ...
            model, scrub_dataset, criterion, args, optim, device, outString=outString
        )
        ...
        if args.train_optim == "sgd":
            ...
            re_optim = torch.optim.Adam(
                re_model.parameters(), lr=args.train_lr, weight_decay=args.train_wd
            )
        else:
        ...
        tmp["retrain_res_loss"] = [
            re_train_loss
        ]  # This is the training loss on the residual set after retraining
        tmp["retrain_res_acc"] = [re_train_accuracy]
        tmp["retrain_res_gradnorm"] = [re_train_gradnorm]

        print(
            f"\t Retraining: Val_loss: {re_val_loss}, Val_acc: {re_val_accuracy}, Res_loss: {re_train_loss}, Res_acc: {re_train_accuracy}, Res_gradnorm: {re_train_gradnorm}"
        )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        type=int,
        default=0,
        help="start point for restarting scrubbing scripts",
    )
