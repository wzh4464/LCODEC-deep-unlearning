...
class NLP_ActivationsHook(nn.Module):
    ...
    def nlplinearhook(self, module, input, output):
        if len(output.shape) == 3:
            flat = output.mean(
                dim=[0, 1]
            )  # mean over tokens? ; output is of shape 1 *n_tokens * 768 (hidden_emb_size), also mean over batches_dim_0
            ...
            self.activations.append(
                output
            )  # for last two linear layers which only utilize the [CLS] token
...
class HyperC(nn.Module):
    ...
    def hook(self, module, input, output):
        if self.interpolate:
            output = F.interpolate(
                output, size=self.interp_size, mode="bilinear", align_corners=True
            )
    ...
    def getHC(self, x):
        ...
        outputs = torch.cat(self.outputs, dim=1) if self.interpolate else self.outputs
