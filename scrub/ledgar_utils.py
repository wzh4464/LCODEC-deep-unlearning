...
class DonData(object):
    ...
    def __init__(self, path):
        ...
        self.label_map = {label: i for i, label in enumerate(self.all_lbls)}
...
def convert_examples_to_features(
    ...
    for ex_ix, example in enumerate(examples):
        ...
        if pad_on_left:
            ...
            input_mask = (
                [0 if mask_padding_with_zero else 1] * pad_length
            ) + input_mask
...
# python3.6
class SplitDataSet:
    def __init__(self, x_train, y_train, x_test, y_test, x_dev=None, y_dev=None):
...
def split_corpus(
    ...
    return SplitDataSet(x_train, y_train, x_test, y_test, x_dev, y_dev)


def evaluate_multilabels(
    y: List[List[str]], y_preds: List[List[str]], do_print: bool = False
    ...
    label_eval: DefaultDict[str, DefaultDict[str, int]] = defaultdict(
        lambda: defaultdict(int)
    )
    assert len(y) == len(
        y_preds
    ), "List of predicted and gold labels are of unequal length"
    ...
    eval_results: DefaultDict[str, Dict[str, float]] = defaultdict(
    ...
    if eval_results["Macro"]["prec"] + eval_results["Macro"]["rec"] == 0:
        ...
        eval_results["Macro"]["f1"] = (
            2 * eval_results["Macro"]["prec"] * eval_results["Macro"]["rec"]
    ...
    if all_fp == 0:
        ...
        if micro_prec + micro_rec == 0:
            ...
            eval_results["Micro"]["f1"] = (2 * micro_rec * micro_prec) / (
                micro_rec + micro_prec
            )
