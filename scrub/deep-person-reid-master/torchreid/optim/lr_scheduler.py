...
def build_lr_scheduler(
    ...
    if lr_scheduler not in AVAI_SCH:
        raise ValueError(
            f"Unsupported scheduler: {lr_scheduler}. Must be one of {AVAI_SCH}"
        )
    ...
    if lr_scheduler == "single_step":
        ...
        if not isinstance(stepsize, int):
            raise TypeError(
                f"For single_step lr_scheduler, stepsize must be an integer, but got {type(stepsize)}"
            )
        ...
        if not isinstance(stepsize, list):
            raise TypeError(
                f"For multi_step lr_scheduler, stepsize must be a list, but got {type(stepsize)}"
            )
