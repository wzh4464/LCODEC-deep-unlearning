...
class OSNet(nn.Module):
        ...
        for dim in fc_dims:
            layers.extend(
                (
                    ...
                    nn.BatchNorm1d(dim, affine=NORM_AFFINE),
    ...
    def build_child_graph(self):
        ...
        for i, conv in enumerate(self.conv2):
            ...
            print(f"- conv2-{i + 1} Block={block.__name__}")
        for i, conv in enumerate(self.conv3):
            block = conv.build_child_graph()
            print(f"- conv3-{i + 1} Block={block.__name__}")
        for i, conv in enumerate(self.conv4):
            block = conv.build_child_graph()
            print(f"- conv4-{i + 1} Block={block.__name__}")
    ...
    def forward(self, x, lmda=1.0, return_featuremaps=False):
        ...
        if self.fc is not None:
            v = self.fc(v)
        return self.classifier(v) if self.training else v
