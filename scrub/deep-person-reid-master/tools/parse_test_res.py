...
def parse_file(filepath, regex_mAP, regex_r1, regex_r5, regex_r10, regex_r20):
    ...
    with open(filepath, "r") as f:
        ...
        for line in lines:
            ...
            if match_mAP := regex_mAP.search(line):
                mAP = float(match_mAP.group(1))
                results["mAP"] = mAP

            if match_r1 := regex_r1.search(line):
                r1 = float(match_r1.group(1))
                results["r1"] = r1

            if match_r5 := regex_r5.search(line):
                r5 = float(match_r5.group(1))
                results["r5"] = r5

            if match_r10 := regex_r10.search(line):
                r10 = float(match_r10.group(1))
                results["r10"] = r10

            if match_r20 := regex_r20.search(line):
