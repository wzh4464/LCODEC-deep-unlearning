...
def getVGGClassLabelFromName(
    person="Aamir_Khan", dataroot="./data/vggface/", namefile="names.txt"
):
...
class VGGFaceDataset(data.Dataset):
    ...
    def __init__(
        self,
        dataroot="./data/vggface/",
        attr_data_file="vggface_metadata.csv",
        person="Aamir_Khan",
        exclude=False,
    ):
        ...
        try:
            self.attr_data = pd.read_csv(
                os.path.join(self.dataroot, self.attr_data_file), low_memory=False
            )
        except FileNotFoundError as e:
            raise ValueError(
                "Image attribute file {:s} does not exist".format(
                    os.path.join(dataroot, self.attr_data_file)
                )
            ) from e
        except e:
            print(e)
        ...
        sampsperclass = 10 if exclude == True else 100
        print("Subsetting persons...")
        tmp = None
        for person in self.attr_data["Person"].unique():
            # print(person)
            if tmp is None:
                tmp = self.attr_data.loc[self.attr_data["Person"] == person][
                    :sampsperclass
                ]
            else:
                tmp = tmp.append(
                    self.attr_data.loc[self.attr_data["Person"] == person][
                        :sampsperclass
                    ],
                    ignore_index=True,
                )
        ...
        if exclude:
            self.person_data = self.attr_data[
                ~(self.attr_data["Person"] == self.person)
            ]
        else:
    ...
    def __getitem__(self, index):
        ...
        im -= torch.Tensor(np.array([129.1863, 104.7624, 93.5940])).view(
            3, 1, 1
        )  # .double().view(3, 1, 1)
