...
class Settings(object):
    ...
    ]  # removal handles of global hooks registered with PyTorch

    def __init__(self):
        assert (
            global_settings_initialized is False
        ), "Reinitializing Settings, seems like a bug."
        self.hook_handles = []
        self.forward_hooks = []
        self.backward_hooks = []
...
def register(model: nn.Module):
    ...
    for layer in model.modules():
        ...
        if _layer_type(layer) in supported_layers:
            layers_registered += 1
            global_settings.hook_handles.append(
                layer.register_forward_hook(_forward_hook)
            )
            layer.register_backward_hook(
                _backward_hook
            )  # don't save handle, https://github.com/pytorch/pytorch/issues/25723
    assert (
        layers_registered
    ), f"Failed to register hooks because model had no supported layers for hooks. Model had layer types {', '.join(list(layer_types_seen))}, but supported types are {', '.join(supported_layers)}"
...
def module_hook(hook: Callable):
    ...
    assert (
        global_settings.hook_handles
    ), "Global hooks have not been registered. Make sure to call .register(model) on your model"
    forward_hook_called = [False]
    backward_hook_called = [False]

    def forward_hook(
        layer: nn.Module, input_: Tuple[torch.Tensor], output: torch.Tensor
    ):
        assert len(input_) == 1, "Only support single input modules on forward."
        assert (
            type(output) == torch.Tensor
        ), "Only support single output modules on forward."
        activations = input_[0].detach()
        hook(layer, activations, output)
        forward_hook_called[0] = True

    def backward_hook(
        layer: nn.Module, input_: Tuple[torch.Tensor], output: Tuple[torch.Tensor]
    ):
    ...
    try:
        yield
        assert (
            forward_hook_called[0] or backward_hook_called[0]
        ), "Hook was called neither on forward nor backward pass, did you register your model?"
        assert not (
            forward_hook_called[0] and backward_hook_called[0]
        ), "Hook was called both on forward and backward pass, did you register your model?"
    finally:
        global_settings.forward_hooks.pop()
        global_settings.backward_hooks.pop()
...
def backward_jacobian(output: torch.Tensor, retain_graph=False) -> None:
    ...
    for idx in range(o):
        output.backward(
            torch.stack([id_mat[idx]] * n), retain_graph=(retain_graph or idx < o - 1)
        )


def backward_hessian(
    output, loss="CrossEntropy", stochastic=False, retain_graph=False
) -> None:
    """
    ...
    assert (
        loss in supported_losses
    ), f"Only {supported_losses} loss is supported, got {loss}"
    ...
    if loss == "CrossEntropy":
        ...
        if not stochastic:
            for out_idx in range(o):
                output.backward(
                    hess_sqrt[:, out_idx, :],
                    retain_graph=(retain_graph or out_idx < o - 1),
                )
        else:
        ...
        if not stochastic:
            ...
            for idx in range(o):
                output.backward(
                    torch.stack([id_mat[idx]] * n),
                    retain_graph=(retain_graph or idx < o - 1),
                )
        else:
            vals = torch.LongTensor(n, o).to(output.device).random_(0, 2) * 2 - 1
