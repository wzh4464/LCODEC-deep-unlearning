...
def main(args):
    ...
    fname = f"celeba_foci_npys/{str(target_index - 1)}_" + args.target + ".npy"
    print(fname)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for image, target_label, spurious_labels in tqdm(train_loader):
            ...
            total_label_accuracy += (
                (target_preds.max(1)[1] == target_label).float().mean().item()
            )
            temp_sp, temp_nsp = calc_classwise_accuracy(
                target_label, spurious_labels, target_preds
            )
        ...
        tqdm.write(f"EPOCH {epoch:03d}: " f"overall_accuracy={overall_accuracy:.4f}")
