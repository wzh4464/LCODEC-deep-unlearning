        ...
        for y_ind in range(n_attrs):
            if cor_type == "pearsonr":
                corr_val, p_val = scipy.stats.pearsonr(
                    attr_data[:, x_ind], attr_data[:, y_ind]
                )
                ...
                corr_val, p_val = scipy.stats.kendalltau(
                    attr_data[:, x_ind], attr_data[:, y_ind]
                )
                corr_matrix[x_ind][y_ind] = np.abs(corr_val)
            else:
...
def grid_plotter(n_attrs, attr_data, no_noise_attr_data, header, fname, args):
    ...
    for i, axi in enumerate(ax.flat):
        ...
        if i == 0:
            svm = sns.heatmap(
                get_corr_matrix(n_attrs, "pearsonr", no_noise_attr_data),
                cmap="YlGnBu",
                xticklabels=False,
                yticklabels=header,
                ax=axi,
            ...
            svm = sns.heatmap(
                get_corr_matrix(n_attrs, "spearmanr", no_noise_attr_data),
                cmap="YlGnBu",
                xticklabels=False,
                yticklabels=False,
                ax=axi,
            ...
            svm = sns.heatmap(
                get_corr_matrix(n_attrs, "pearsonr", attr_data),
                cmap="YlGnBu",
                xticklabels=False,
                yticklabels=header,
                ax=axi,
            ...
            svm = sns.heatmap(
                get_corr_matrix(n_attrs, "spearmanr", attr_data),
                cmap="YlGnBu",
                xticklabels=False,
                yticklabels=False,
                ax=axi,
...
fname = f"attr_cor_noise_0_and_{str(args.noise)}.png"
...
sin_fig = svm.get_figure()
sin_fig.savefig(
    f"Pearson_noise_{str(args.noise)}.png",
    dpi=args.quality,
    bbox_inches="tight",
)
...
sin_fig = svm.get_figure()
sin_fig.savefig(
    f"Spearman_noise_{str(args.noise)}.png",
    dpi=args.quality,
    bbox_inches="tight",
)
...
sin_fig = svm.get_figure()
sin_fig.savefig(
    f"CODEC_noise_{str(args.noise)}.png", dpi=args.quality, bbox_inches="tight"
)
